...
public:
	...
	void set_strateg_position(AStrategist* strateg_ptr, Location loc, AMapPainter* painter_ptr);
	...
	AI_MoveModel(AStrategist* strateg_ptr, const TArray<AStructure*>& structures, AMyPlayerCharacter* player_ptr, GameMap* map_ptr, AGameWorld* game_world) {
		...
		if (nearest_structure.IsSet()) {
			if (step_to_location(strateg_ptr, nearest_structure.GetValue()->position, map_ptr, game_world->painter_ptr)) {
				return;
			}
		}
		random_step(strateg_ptr, map_ptr, game_world->painter_ptr);
	}
};
